...
|---|---|---|---|---|
| 1 | Agregar `moveSequence: CellPosition[]` a `GameSession` | EXPAND-ELEMENT | PENDING | `gameSession.ts` — registro acumulativo de movimientos |
| 2 | Implementar `verifyBoardIntegrity(session)` | GENERATE-DOCUMENT | PENDING | `gameSession.ts` — replay desde seed, retorna `boolean` |
| 3 | Agregar `verified: boolean` a `GameResultRecord` | EXPAND-ELEMENT | PENDING | `gameLocalStore.ts` |
| 4 | Llamar `verifyBoardIntegrity` en victoria antes de acreditar monedas | EXPAND-ELEMENT | PENDING | `ResultPage.tsx` — si `!verified`, no acreditar coins ni mostrar botón ranking |
| 5 | Firma HMAC-SHA256 ligera sobre resultado (seed + score + moves) | GENERATE-DOCUMENT | PENDING | `integrityService.ts` — `Web Crypto API`; firma almacenada en el record |
| 6 | Validar firma al leer resultados del ranking | EXPAND-ELEMENT | PENDING | `gameLocalStore.ts` — filtrar records sin firma válida |
| 7 | Plausibility check de timestamps (velocidad mínima por movimiento) | EXPAND-ELEMENT | PENDING | `gameSession.ts` — `MIN_MS_PER_MOVE = 200` |
| 8 | Tests de integridad: replay válido, replay alterado, firma inválida | GENERATE-DOCUMENT | PENDING | `boardIntegrity.test.ts` |
...
- [ ] Un tablero ganado por replay de movimientos reales supera la verificación.
- [ ] Un resultado con `moveSequence` alterada es rechazado por `verifyBoardIntegrity`.
- [ ] Records con firma HMAC inválida no aparecen en el ranking.
- [ ] El tiempo total / número de movimientos no viola el umbral mínimo.
- [ ] Tests cubren escenarios válidos e inválidos.
